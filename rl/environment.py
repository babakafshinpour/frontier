...
from cytokine_model_tiny import Cytokine1 
from cytokine_model_normalized import CytokineNormalized 
import numpy as np
from dynamic_type_model import DT_GRU
from drug_efficacy_model import DE_GRU
import torch
...
class CustomEnv(gym.Env):
    ...
    def __init__(self, steriod_max, observation_min,observation_max,step_size, eps_perb=0,behaviour = None, cytokine_model="normalized", **kwargs):
        ...
        self.action_space = spaces.Box(low=np.array([0]),
                                       high=np.array([steriod_max])) # 3 grams per day ?
        
        # Example for using image as input:
        self.observation_space = spaces.Box(low=np.array(observation_min), 
                                            high=np.array(observation_max)) # e.g. TNF-alpha
    ...
    def step(self, action_in):
        ...
        observation,old_observation = self.cytokine.step_to(self.current_time+self.step_size)
    ...
    def get_dynamic_drug_eff(self):
        ...
        normalizer = np.cumsum(temp,axis=0)/np.arange(1,temp.shape[0]+1,1)[:,np.newaxis]
    ...
    def reset(self,p0):
        ...
        all_obs_list = self.cytokine.current_values.tolist()# + self.cytokine.current_values.tolist() + [0]
        observation = np.array(all_obs_list)
        ...
        #pdb.set_trace()
        dynamic_type,drug_eff = self.get_dynamic_drug_eff()
        observation = np.array(observation[:-1].tolist()+[0]+dynamic_type+drug_eff)
        return observation
